...
class BSTRAND : public Map<K,V,comparison_fn, equality_fn>
{
private:
  struct Node
  {
...
public:
  ...
  void r_insert_at_root(Node *&rt, K key, V val);
  void r_insert_at_leaf(Node *&rt, K key, V val);
  V remove(K key);
  V& lookup(K key);
  bool is_empty();
  void print_inorder();
...
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
BSTRAND<K, V, comparison_fn, equality_fn>::Node::Node(K key_a, V val_a) : key(key_a), val(val_a) {}


// --- constructor --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
BSTRAND<K, V, comparison_fn, equality_fn>::BSTRAND()
{
...
// --- print_root --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
void BSTRAND<K, V, comparison_fn, equality_fn>::print_root()
{
...
// --- insert --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
void BSTRAND<K, V, comparison_fn, equality_fn>::insert(K key, V val)
{
  int random_integer = rand()%(size() + 1);
  if (random_integer == 0){
    std::cout << "Insert " << val << " at root" << std::endl;
    r_insert_at_root(root, key, val);
  } else {
    std::cout << "Insert " << val << " at leaf" << std::endl;
    r_insert_at_leaf(root, key, val);
  }
}

// --- r_insert_at_leaf --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
void BSTRAND<K, V, comparison_fn, equality_fn>::r_insert_at_leaf(Node *&rt, K key, V val)
{
  ...
  if (!rt){
    ...
    if (comparison_fn(key, rt->key)){
      r_insert_at_leaf(rt->l, key, val);
    }
    else {
      r_insert_at_leaf(rt->r, key, val);
    }
  }
...
// --- r_insert_at_root --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
void BSTRAND<K, V, comparison_fn, equality_fn>::r_insert_at_root(Node *&rt, K key, V val)
{
  if (!rt){
    rt = new Node(key, val);
    return;
    ...
    if (comparison_fn(key, rt->key)){
      r_insert_at_root(rt->l, key, val);
      rotate_right(rt);
    }
    else {
      r_insert_at_root(rt->r, key, val);
      rotate_left(rt);
    }
  }
}
...
// --- rotate_left --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
void BSTRAND<K, V, comparison_fn, equality_fn>::rotate_left(Node *&rt)
{
...
// --- rotate_right --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
void BSTRAND<K, V, comparison_fn, equality_fn>::rotate_right(Node *&rt)
{
...
// --- is_empty --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
bool BSTRAND<K, V, comparison_fn, equality_fn>::is_empty()
{
  return (!root);
}
...
// --- print_inorder --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
void BSTRAND<K, V, comparison_fn, equality_fn>::print_inorder()
{
...
// --- r_ print_inorder --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
void BSTRAND<K, V, comparison_fn, equality_fn>::r_print_inorder(Node *&rt)
{
...
// --- find_node --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
typename BSTRAND<K, V, comparison_fn, equality_fn>::Node*& BSTRAND<K, V, comparison_fn, equality_fn>::find_node(Node *&rt, K key)
{
  if (!rt) // if we have gotten to the child of a leaf node, throw an error
    throw std::runtime_error("BSTRAND.find_node(): key not found");
...
// --- lookup --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
V& BSTRAND<K, V, comparison_fn, equality_fn>::lookup(K key)
{
...
// --- remove --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
V BSTRAND<K, V, comparison_fn, equality_fn>::remove(K key)
{
...
// --- ext_remove --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
void BSTRAND<K, V, comparison_fn, equality_fn>::ext_remove(Node *&rt)
{
...
// --- smallest_node --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
typename BSTRAND<K, V, comparison_fn, equality_fn>::Node*& BSTRAND<K, V, comparison_fn, equality_fn>::smallest_node(Node *&rt)
{
...
// --- num_children --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
int BSTRAND<K, V, comparison_fn, equality_fn>::num_children(Node *curr)
{
...
// --- find_parent --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
typename BSTRAND<K, V, comparison_fn, equality_fn>::Node* BSTRAND<K, V, comparison_fn, equality_fn>::find_parent(Node *&rt, K key)
{
  // if it is root, no parent so return nullptr
  // if (rt == this->root) {
...
// --- is_l_of_parent --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
bool BSTRAND<K, V, comparison_fn, equality_fn>::is_l_of_parent(Node *child, Node *parent)
{
  if (parent->l)
  {
...
// --- has_left --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
bool BSTRAND<K, V, comparison_fn, equality_fn>::has_left(Node *curr)
{
  return curr->l;
}
...
// --- has_right --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
bool BSTRAND<K, V, comparison_fn, equality_fn>::has_right(Node *curr)
{
  return curr->r;
}
...
// --- size --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
size_t BSTRAND<K, V, comparison_fn, equality_fn>::size()
{
...
// --- r_size --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
size_t BSTRAND<K, V, comparison_fn, equality_fn>::r_size(Node *rt)
{
  size_t num_nodes = 1;
  if (rt->l)
...
// --- contents --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
V* BSTRAND<K, V, comparison_fn, equality_fn>::contents()
{
...
// --- r_contents --- //
template <typename K, typename V, bool (*comparison_fn)( K key1,  K key2 ), bool (*equality_fn)( K key1 ,  K key2)>
void BSTRAND<K, V, comparison_fn, equality_fn>::r_contents(Node *rt, V* arr, size_t &i)
{
